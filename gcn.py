...
#import model
import layers
from modules.data_util import load_and_split_data, load_data, split_data
from modules.core import CoreModel
...
def plot_cost(config,data,model,prefix=""):
    from modules.make_plots import make_cost_acc_plot
...
def plot_auc(config,labels,pred_data,prefix=""):
    from modules.make_plots import make_auc_plot,make_multitask_auc_plot
    result_path = config["plot_path"]
    os.makedirs(result_path, exist_ok=True)
...
def plot_r2(config,labels,pred_data,prefix=""):
    from modules.make_plots import make_r2_plot
    result_path = config["plot_path"]
    os.makedirs(result_path, exist_ok=True)
    if config["plot_multitask"]:
        print("not supported")
...
def train_cv(sess,graph,config):
    ...
    from modules.make_plots import make_auc_plot, make_cost_acc_plot
    import sklearn
...
def infer(sess,graph,config):
    ...
    all_data,info=load_data(config,filename=dataset_filename,test_mode=True)
    ...
    if config["save_info_test"] is not None:
        ...
        result["prediction_data"]=pred_score
        if len(pred_score.shape)==3: # multi-label-multi-task
            # #data x # task x #class
            # => this program supports only 2 labels
            pred_score=pred_score[:,:,1]
        ...
        if ext==".json":
            ...
            joblib.dump(result,save_path,compress=True)
...
def visualize(sess, config, args):
    ...
    from modules.visualization import cal_feature_IG, cal_feature_IG_for_kg
    # 入力は１分子づつ
    batch_size = 1
    ...
    ## load a model with 'feed_embedded_layer=False' to feed emmbedded layer as learnable parameters to a session.
    model = importlib.import_module(config["model.py"])
    ...
    try:
        ...
        ## change embedded_layer from learnable to static.
        placeholders = model.build_placeholders(info, config, batch_size=batch_size, feed_embedded_layer=True)
        _model, prediction, _, _, _ = model.build_model(placeholders, info, config, batch_size=batch_size, feed_embedded_layer=True)
        saver = tf.train.Saver()
        saver.restore(sess, config["load_model"])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--visualize_node0', type=int,
            default=None,
            help="node number at one end of the target edge")
    parser.add_argument('--visualize_node1', type=int,
            default=None,
            help="node number at another end of the target edge")
    ...
    config["visualize_node0"] = args.visualize_node0
    config["visualize_node1"] = args.visualize_node1
